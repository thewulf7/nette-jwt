...
class JWTUserStorage implements IUserStorage
{
								...
								IIdentitySerializer $identitySerializer)
	{
		...
		$this->COOKIE_NAME = session_name() . '-jwt';
	}
	...
	private function saveJWTCookie()
	{
		if (empty($this->jwtData)) {
			$this->response->deleteCookie($this->COOKIE_NAME);
			return;
		}
		...
		$this->response->setCookie($this->COOKIE_NAME, $jwt, $this->expirationTime);
	...
	private function loadJWTCookie()
	{
		...
		$jwtCookie = $this->request->getCookie($this->COOKIE_NAME);
		if (!$jwtCookie) {
